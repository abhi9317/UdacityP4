...
There are 1599 wines in this dataset. Most wines seem to have a quality rating of 5 or 6 and the range of ratings are from 3-8 implying that no wine expert rated any of the wines on the extreme ends of the scales. ie. very low(0,1,2) and very high(9,10).

75% of wines have a pH rating below 3.4 which implies that most wines are atleast moderately acidic.

75% of wines have 21mg or lower concentrations of free sulfur dioxide in them.

The median density of the wines is 0.9968 which is very close to the density of water

The mean alcohol content in the wines of the dataset is 10.42%

Since the guiding question for this analysis is 'Which chemical properties influence the quality of red wines?', I wanted to look at the distribution of quality first.
...
precise). The distribution of the quality ratings of the wines seems to be somewhat normal. 

Next I thought it would be worthwhile to look at the distributions of all the variables to learn more 
about the distribution of the data. 

## Fixed Acidity
```{r echo=FALSE, Univariate_Plots_3}
#This plot depicts the distibution of fixed acidity (in g/dm3) in the wines given in the data.
...
Generally most wines seem to have a fixed acidity level between 7-8 but the range of values extends from around 4 till 16 g/dm3

## Volatile Acidity
```{r echo=FALSE, Univariate_Plots_4}
#This plot depicts the distibution of volatile acidity content in the wines given in the data.
...
remove_outliers <- function(dframe,column){
  result <- subset(dframe, (column < quantile(column,probs = (0.75)) + (1.5*IQR(column))) & (column > quantile(column, probs = 0.25) - (1.5 * IQR(column))))
  return (result)
}

#Since the first plot had a slight tail, I decided to remove the upper outliers to get a better look at the underlying distribution
...
After removing outliers above around 1 (the exact value was determined with the IQR and 75th percentile), we see that the distribution of volatile acidity looks bi modal with 2 peaks around 0.4 and 0.6

## Citric Acid
```{r echo=FALSE, Univariate_Plots_5}
#This plot depicts the distibution of citric acid content in the wines given in the data.
...
For citric acid content, there seems to be a peaks around 0 and 0.49 g/dm3 but there is also a clear outlier at 1g/dm3. Apart from that most of the values lie between 0 and 0.75
...
After removing outliers, the distribution of residual sugars in the wines of the dataset looks normally distributed

## Chlorides
```{r echo=FALSE, Univariate_Plots_7}
#This plot depicts the distibution of salt (in g/dm3) in the wines given in the data.
...
Once again, after removing outliers, the distribution of cholorides in the wine looks like it is normally distributed

## Free Sulfur Dioxide
```{r echo=FALSE, Univariate_Plots_8}
#This plot depicts the distibution of free sulfur dioxide (in g/dm3) in the wines given in the data.
...
After transforming the distribution to log scale (given its long tail nature), it looks like there is a peak at 6 g/dm3 and that most of the sulfur dioxide values are whole numbers 

## Total Sulfur Dioxide
```{r echo=FALSE, Univariate_Plots_9}
#This plot depicts the distibution of total sulfur dioxide (in g/dm3) in the wines given in the data.

ggplot(aes(total.sulfur.dioxide), data = remove_outliers(rw,rw$total.sulfur.dioxide))+
...
After removing outliers and transforming the distribution (due to the long tail nature) we can see that the distribution has a slight peak at around 50. 
...
corrplot(corr_rw, method="square",tl.cex = .40, cl.cex = 0.6, title = "Correlation Matrix for red wine dataset", mar=c(0,0,1,0))
```

### Description Two
Here we can see the correlations between all the features of the dataset. In particular, if we look at the feature of interest (quality) we'll notice that alcohol and volatile acidity are the strongest relationships. 
